...
export class SharedLessonService {
  ...
  static async getComments(
    ...
    try {
      ...
      // Map session_id to practice_id for compatibility with PracticeComment
      return (data || []).map(comment => ({
        ...comment,
        practice_id: comment.session_id
      }))
    } catch (error) {
      console.error('Error fetching comments:', error)
      return []
...
export interface RecordingComment {
  id: string
  session_id: string
  practice_id: string  // Compatibility alias for session_id (for PracticeComment compatibility)
  recording_id: string
  user_name: string
  user_email?: string
  user_id?: string  // Compatibility field for PracticeComment
  comment_text: string
  timestamp_seconds?: number
  parent_comment_id?: string | null  // Compatibility field for threaded comments
  created_at: string
  updated_at: string
}