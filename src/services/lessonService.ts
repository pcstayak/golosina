...
export class LessonService {
  ...
  static async createPracticeSession(data: {
    lesson_id: string;
    ...
    try {
      const sessionId = `lesson_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      ...
      if (sessionError || !sessionData) {
        ...
        return { success: false, error: 'Failed to create practice session' };
      }

      return { success: true, sessionId: sessionData.session_id };
    } catch (error) {
  ...
  static async savePracticeRecordings(
    ...
    try {
      // Upload recordings to storage and save metadata
      const recordingCount = Object.keys(recordings).reduce(
        (count, key) => count + (recordings[key]?.length || 0),
        0
      );

      // TODO: Implement actual file upload to Supabase storage
      // For now, just update the session with recording metadata
  ...
  static async getPracticeSession(sessionId: string): Promise<any> {
    if (!supabase) {
      console.error('Supabase is not configured');
      return null;
    }
  ...
  static async deleteLessonPracticeSession(sessionId: string): Promise<{
    success: boolean;
    error?: string;
  }> {
    if (!supabase) {
    ...
    try {
      ...
      console.error('Error deleting lesson practice session:', error);
      return {
        success: false,
